...
class Versions(Example):
    ...
    def get_version(self):
        self.snippet(
            "versions",
            "/v1.0", "GET", 200, "OK",
            lambda client: client.versions.index(conf['version_url'] + "/v1.0/"))
...
class Databases(Example):
    ...
    def get_list_databases_limit_two(self):
        results = self.snippet(
            ...
            "GET", 200, "OK",
            lambda client:
                client.databases.list(json_instance.id, limit=1))
...
class Users(Example):
    ...
    def delete_users(self):
        ...
        self.snippet(
            ...
            "DELETE", 202, "Accepted",
            lambda client:
                client.users.delete(json_instance.id, username=user_name))
...
class Datastores(Example):
    ...
    def get_datastore_by_id(self):
        json,  = self.datastores
        mysql_ds = [x for x in json if x.name == 'mysql']
        if not mysql_ds:
    ...
    def get_datastore_version_by_id(self):
        ds_id = STATE["DATASTORE_ID"]
        ds_v_id = STATE["DATASTORE_VERSION_ID"] = self.datastore_versions[JSON_INDEX][0].id
...
class Configurations(ActiveMixin):
    ...
    def get_configuration_parameters_for_datastore_version(self):
        ...
        self.snippet(
            ...
            "GET", 200, "OK",
            lambda client:
                client.configuration_parameters.parameters(ds_id, ds_v_id))
    ...
    def get_configuration_parameter_without_datastore_version(self):
        ...
        self.params = self.snippet(
            ...
            "GET", 200, "OK",
            lambda client: client.configuration_parameters.get_parameter_by_version(ds_v_id, param))
    ...
    def attach_configuration_to_instance(self):
        ...
        self.snippet(
            ...
            "PUT", 202, "Accepted",
            lambda client: client.instances.modify(json_instance.id, config.id))
    ...
    def mgmt_list_hosts(self):
        ...
        with Check() as check:
            for hosts in results:
                ...
                check.equal("fake_host_1", hosts[1].name)
                check.equal("fake_host_2", hosts[0].name)
                check.equal(1, results[0][1].instanceCount)
...
class MgmtInstanceRoot(Example):
    ...
    def mgmt_get_root_details(self):
        self.snippet(
            ...
            "GET", 200, "OK",
            lambda client:
                client.mgmt.instances.root_enabled_history(json_instance.id))
