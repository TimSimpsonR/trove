...
class SnippetWriter(object):
    ...
    def _write_file(self, filename, output):
        def goofy_diff(original, current):
            a = original.split("\n")
            b = current.split("\n")
            diff = []
            for i in range(len(a)):
                if i < len(b):
                    ...
                    diff.append("%d -%s" % (i, a[i]))
            for j in range(len(b) - len(a)):
                i2 = len(a) + j
                diff.append("%d +%s" % (i, b[i2]))
            return diff
        ...
        if os.path.isfile(filename):
            ...
            if len(output) == 0:
                fail('Error: output missing in new snippet generation for %s. '
                     'Old content follows:\n"""%s"""' % (filename, original))
            if original != output:
                diff = '\n'.join(goofy_diff(original, output))
